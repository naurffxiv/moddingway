...
async def get_user_notes(
    ...
    for note in note_list:
        result = (
            result
            + f"\n* ID: {note.note_id} | Note: {note.content} | Note Creator: <@{note.created_by}> | Last Editor: <@{note.last_editor}>"
        )
...
async def update_user_note(
    ...
    result = notes_database.update_note(new_note, str(last_author.id), note_id)
    if result:
        logging_embed.set_footer(text=f"Note ID: {note_id}")
